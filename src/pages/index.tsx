...
export default function Home() {
  ...
  async function handleIngest(url: string) {
    setIndexingRepos(prev => new Set(prev).add(url));
    try {
      ...
      toast.error(err instanceof Error ? err.message : "An error occurred during indexing");
    } finally {
                ...
                <tbody>
                  {userRepos.map((repo: { svn_url: string; full_name: string; }, index) => (
                    <tr key={index} className="border-t border-gray-200 dark:border-zinc-700">
                      ...
                      <td className="px-4 py-2">
                        <Button
                          onClick={async () => await handleIngest(repo.svn_url)}
                          disabled={indexingRepos.has(repo.svn_url)}
                          className="w-fit"
                        >
                          {indexingRepos.has(repo.svn_url) ? "Indexing..." : "Ingest"}
                        </Button>
                      </td>
                    </tr>
                  ))}
